...
class TestDiff(unittest.TestCase):
    ...
    def test_doc_string_parameter(self):
        ...
        self.assertRegexpMatches(
            sig_power.__doc__, "Parameter", "didn't find `Parameters` in doc string"
        )
    ...
    def test_doc_string_return(self):
        ...
        self.assertRegexpMatches(
            sig_power.__doc__, "Return", "didn't find `Returns` in doc string"
        )
    ...
    def test_real_power(self):
        """Is the power of a complex signal real-valued?"""
        tt = np.arange(0, 10, 1 / 100)
        xx = np.exp(2j * np.pi * 3 * tt)
        self.assertTrue(
            isinstance(sig_power(xx), float), msg="is power of complex signal real?"
        )
